<x-game-layout :title="'Classement - '.$quiz->title">
    <div class="mb-4">
        <a href="{{ route('quiz.show', $quiz->slug) }}" class="text-sm text-rose-600">← Retour au quiz</a>
    </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-3xl p-6 shadow-sm">
        ...
        <div class="space-y-3">
            ...
            <div class="flex items-center gap-4 p-3 rounded-2xl {{ $index === 0 ? 'bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-700' : 'bg-gray-50 dark:bg-gray-700/40' }}">
                <span class="w-8 text-center font-bold text-lg {{ $index === 0 ? 'text-amber-500' : ($index === 1 ? 'text-gray-400' : ($index === 2 ? 'text-orange-400' : 'text-gray-400')) }}">
                ...
                <span class="font-extrabold text-rose-600">{{ $attempt->score }}/{{ $attempt->total }}</span>
            </div>
            @endforeach
        </div>
        @else
