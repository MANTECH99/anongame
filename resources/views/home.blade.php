<x-game-layout title="Accueil">
    ...
    <section class="rounded-3xl overflow-hidden relative mb-8">
        <div class="absolute inset-0 bg-gradient-to-br from-rose-600 via-pink-600 to-purple-700"></div>
        <div class="relative px-6 py-10 sm:py-14 text-white">
            <h1 class="text-3xl sm:text-4xl font-extrabold leading-tight mb-3">Joue, défie et partage <span class="text-amber-300">anonymement</span></h1>
            ...
            <div class="flex gap-2 sm:gap-3">
                <a href="{{ route('quiz.index') }}" class="flex-1 bg-white text-rose-600 font-semibold px-3 sm:px-5 py-2.5 rounded-xl hover:bg-rose-50 transition whitespace-nowrap text-center text-sm sm:text-base">🎯 Jouer à un quiz</a>
    ...
    <section class="grid grid-cols-3 gap-3 mb-8">
        <div class="bg-white dark:bg-gray-800 rounded-2xl p-4 text-center shadow-sm">
            <p class="text-2xl font-extrabold text-rose-600">{{ number_format($stats['quizzes']) }}</p>
            <p class="text-xs text-gray-500">Quiz</p>
        </div>
        <div class="bg-white dark:bg-gray-800 rounded-2xl p-4 text-center shadow-sm">
            <p class="text-2xl font-extrabold text-purple-600">{{ number_format($stats['devinettes']) }}</p>
            <p class="text-xs text-gray-500">Devinettes</p>
        </div>
        <div class="bg-white dark:bg-gray-800 rounded-2xl p-4 text-center shadow-sm">
            <p class="text-2xl font-extrabold text-amber-500">{{ number_format($stats['players']) }}</p>
    ...
    <section class="mb-8">
        <div class="flex items-center justify-between mb-3">
            ...
            <a href="{{ route('quiz.index') }}" class="text-sm font-semibold text-rose-600">Voir tout →</a>
        </div>
        <div class="grid gap-4 sm:grid-cols-2">
            ...
            <a href="{{ route('quiz.show', $quiz->slug) }}" class="bg-white dark:bg-gray-800 rounded-2xl p-5 shadow-sm hover:shadow-md transition">
                <div class="flex items-start justify-between gap-2">
                    ...
                    <span class="shrink-0 text-xs bg-rose-100 text-rose-700 px-2 py-1 rounded-full">{{ $quiz->category }}</span>
                </div>
    ...
    <section>
        <div class="flex items-center justify-between mb-3">
            ...
            <a href="{{ route('devinette.index') }}" class="text-sm font-semibold text-rose-600">Voir tout →</a>
        </div>
        <div class="grid gap-4 sm:grid-cols-2">
