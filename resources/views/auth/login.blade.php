    ...
    <form method="POST" action="{{ route('login') }}" class="space-y-5">
        ...
        <div>
            <div class="flex items-center justify-between mb-1.5">
                ...
                <a href="{{ route('password.request') }}" class="text-xs font-medium text-rose-600 hover:text-rose-700">Mot de passe oublié ?</a>
                @endif
            </div>
            <div class="relative">
                <input x-data x-ref="password" id="password" type="password" name="password" required autocomplete="current-password"
                    placeholder="••••••••"
        ...
        <label for="remember_me" class="inline-flex items-center gap-2.5">
            <input id="remember_me" type="checkbox" name="remember" class="rounded border-gray-300 text-rose-600 shadow-sm focus:ring-rose-500 w-4 h-4">
            <span class="text-sm text-gray-600">Se souvenir de moi</span>
        </label>

        <button type="submit"
            class="w-full bg-gradient-to-r from-rose-600 to-pink-600 hover:from-rose-700 hover:to-pink-700 text-white font-bold py-3 rounded-xl transition shadow-lg shadow-rose-600/25">
            Se connecter
        </button>
    </form>
